...
public class AiWeeklyReportServiceImpl implements AiWeeklyReportService {
    ...
    public AmazonWeeklyReportDO generateReport(Long shopId, String reportWeek) {
        // 检查是否已存在
        var existing = reportMapper.selectByWeek(shopId, reportWeek);
        if (existing != null && existing.getStatus() == 1) {
            return existing;
        }

        var report = existing != null ? existing : new AmazonWeeklyReportDO();
        ...
        // TODO: 从各模块聚合数据
        var data = new AmazonWeeklyReportDO.ReportData();
        ...
        report.setAiRecommendations(List.of("待 AI 生成"));
        report.setStatus(1);
